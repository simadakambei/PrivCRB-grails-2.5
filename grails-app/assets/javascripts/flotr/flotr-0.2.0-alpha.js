...
var Flotr = {version: "0.2.0-alpha", author: "Bas Wenneker", website: "http://www.solutoire.com", _registeredTypes: {lines: "drawSeriesLines", points: "drawSeriesPoints", bars: "drawSeriesBars", candles: "drawSeriesCandles", pie: "drawSeriesPie"}, register: function (A, B) {
...
Flotr.Graph = Class.create({initialize: function (B, C, A) {
    ...
    var A = new Element("div", {className: "flotr-tabs-group", style: "position:absolute;left:0px;top:" + this.canvasHeight + "px;width:" + this.canvasWidth + "px;"});
    ...
    var A = new Element("div", {className: "flotr-datagrid-container", style: "left:0px;top:0px;width:" + this.canvasWidth + "px;height:" + this.canvasHeight + "px;overflow:auto;"});
    A.insert(H);
    K.wrap(A.hide());
    this.el.insert(A);
    return K
    ...
    if (!this.options.pie.drawn) {
        var K = this.ctx, C = this.options, E = G.pie.lineWidth, I = G.shadowSize, R = G.data, D = (Math.min(this.canvasWidth, this.canvasHeight) * G.pie.sizeRatio) / 2, H = [];
        var L = 1;
        ...
        var A = O.collect(function (S) {
            ...
            F = J / B;
            return{name: S.name, fraction: F, x: S.value[0], y: J, explode: S.explode, startAngle: 2 * N * Math.PI, endAngle: 2 * (N + F) * Math.PI}
        });
        K.save();
        if (I > 0) {
            A.each(function (V) {
    ...
    if (Q) {
        if (!B.HtmlText && this.textEnabled) {
            ...
            var M = B.legend.position, N = B.legend.margin, L = B.legend.labelBoxWidth, Z = B.legend.labelBoxHeight, S = B.legend.labelBoxMargin, W = I.left + N, U = I.top + N;
            var a = 0;
            ...
            if (b.length > 0) {
                ...
                if (B.legend.container != null) {
                    ...
                    if (B.legend.backgroundOpacity != 0) {
                        ...
                        this.el.insert('<div class="flotr-legend-bg" style="position:absolute;width:' + J.getWidth() + "px;height:" + J.getHeight() + "px;" + D + "background-color:" + Y + ';"> </div>').select("div.flotr-legend-bg").first().setStyle({opacity: B.legend.backgroundOpacity})
                    }
                }
            }
        }
    ...
    return{x: this.axes.x.min + F / this.axes.x.scale, x2: this.axes.x2.min + F / this.axes.x2.scale, y: this.axes.y.max - E / this.axes.y.scale, y2: this.axes.y2.max - E / this.axes.y2.scale, relX: F, relY: E, absX: D, absY: B}
    ...
    var A = this.axes, F = this.selection, C = (F.first.x <= F.second.x) ? F.first.x : F.second.x, B = (F.first.x <= F.second.x) ? F.second.x : F.first.x, E = (F.first.y >= F.second.y) ? F.first.y : F.second.y, D = (F.first.y >= F.second.y) ? F.second.y : F.first.y;
    ...
    var G = this.prevSelection, E = this.octx, C = this.plotOffset, A = Math.min(G.first.x, G.second.x), F = Math.min(G.first.y, G.second.y), B = Math.abs(G.second.x - G.first.x), D = Math.abs(G.second.y - G.first.y);
    E.clearRect(A + C.left - E.lineWidth, F + C.top - E.lineWidth, B + E.lineWidth * 2, D + E.lineWidth * 2);
    this.prevSelection = null
}, setSelection: function (G) {
    var B = this.options, H = this.axes.x, A = this.axes.y, F = yaxis.scale, D = xaxis.scale, E = B.selection.mode.indexOf("x") != -1, C = B.selection.mode.indexOf("y") != -1;
    this.clearSelection();
    ...
    var E = Math.min(F.first.x, F.second.x), D = Math.min(F.first.y, F.second.y), G = Math.abs(F.second.x - F.first.x), B = Math.abs(F.second.y - F.first.y);
...
Flotr.Date = {format: function (F, E) {
    ...
    return D.join("")
}, timeUnits: {second: 1000, minute: 60 * 1000, hour: 60 * 60 * 1000, day: 24 * 60 * 60 * 1000, month: 30 * 24 * 60 * 60 * 1000, year: 365.2425 * 24 * 60 * 60 * 1000}, spec: [
    [1, "second"],
    [2, "second"],
    [5, "second"],
    [10, "second"],
